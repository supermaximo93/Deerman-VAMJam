...
.requires(
	...
	'game.levels.level1',
	window.DEBUG ? 'impact.debug.debug' : 'game.entities.tree'
)
.defines(function() {
...
LoseScreen = ig.Game.extend({
	goddeath: new ig.Image('media/goddeath.png'),
	titleImage: new ig.Image('media/losescreen.png'),
	lion: new ig.Image('media/liondeath.png'),
	verb: 'LASTED',
...
GameScreen = ig.Game.extend({
	...
	depthSortEntities: function() {
		for (var i = 0; i < this.entities.length; ++i) {
			...
			entity.zIndex = entity.pos.y + entity.size.y + entity.offset.y;
		}
		this.sortEntities();
	},
	...
	levelFitsInView: function() {
		return this.levelWidth() <= SCREEN_SIZE.x && this.levelHeight() <= SCREEN_SIZE.y;
	},

	setCameraWithinLevelBounds: function() {
		ig.game.screen.x = ig.Utils.clamp(0, this.levelWidth() - SCREEN_SIZE.x);
		ig.game.screen.y = ig.Utils.clamp(0, this.levelHeight() - SCREEN_SIZE.y);
	},
