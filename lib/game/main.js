...
GameScreen = ig.Game.extend({
	...
	levelFitsInView: function() {
		var xFits = this.collisionMap.data[0].length * this.collisionMap.tilesize <= SCREEN_SIZE.x;
		var yFits = this.collisionMap.data.length * this.collisionMap.tilesize <= SCREEN_SIZE.y;
		return xFits && yFits;
	}
});
